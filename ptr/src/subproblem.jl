...
function solveSubproblem!(p::ptr)
    ...
    # objective = (sumsquares(u) + p.wD * norm(Δ) + p.wnu * norm(nu, 1)) / p.wnu
    ...
    # constraints = Constraint[
    #     # x[:, k+1] == p.A[:, :, k] * x[:, k] + p.A[:, 4:6, k] * u[:, k] + nu[(k-1)*p.nx+1:k*p.nx] for k in 1:p.K-1
    #     # x[:, k+1] == p.A[:, :, k] * x[:, k] + p.A[:, 4:6, k] * u[:, k] for k in 1:p.K-1
    # ]
